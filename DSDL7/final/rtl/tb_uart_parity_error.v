...
module tb_uart_parity_error;
    ...
    initial begin
        $dumpfile("/tmp/tb_uart_parity_error.vcd");
        $dumpvars(0, tb_uart_parity_error);
        rstN = 1'b1;
        rx = 1'b1;
        #1 rstN = 1'b0;
        ...
        drive_frame(BAD_DATA, ~(^BAD_DATA), 1'b1);
        wait_completion;
        if (correct_data !== 1'b0)
            $fatal(1, "FAIL: corrupted parity was accepted");
        if (rec_data !== SEED_DATA)
            $fatal(1, "FAIL: invalid frame changed rec_data to %b", rec_data);
        @(negedge clk);
        if (rec_new_data !== 1'b0)
            $fatal(1, "FAIL: rec_new_data exceeded one cycle");

        $display("PASS: parity errors are rejected");
        $finish;
    end
