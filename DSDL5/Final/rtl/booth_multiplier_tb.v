// Self-checking directed and exhaustive testbench for the N=8 multiplier.
// Waveforms cover directed cases only; all 65,536 pairs are still checked.
...
module booth_multiplier_tb;
    ...
    // Count datapath calculation cycles, excluding load/report overhead.
    always @(posedge Clk)
        if (dut.Step) RunSteps = RunSteps + 1;

    // Run one multiplication and compare it with Verilog's signed multiply.
    task do_mul;
        input reg [N-1:0] a;
        input reg [N-1:0] b;
        begin
    ...
    // Print directed cases for use in the report.
    task show_mul;
        input reg [N-1:0] a;
        input reg [N-1:0] b;
